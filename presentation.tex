...
\section{The EVM}
\begin{frame}{What is the EVM?}
The Ethereum Virtual Machine (EVM) is the \textbf{execution environment} for
smart contracts on Ethereum. It is:
\begin{itemize}
    \item \textbf{stack machine} -- instead of registers, it uses a stack
    \item \textbf{bytecode-based} -- it executes bytecode instructions
    \item \textbf{deterministic} -- it produces the same result for the same input \& state
    \item \textbf{isolated} -- it runs in a sandboxed environment
    \item \textbf{gas-based} -- it charges for execution based on gas consumption
    \item \textbf{ACID-compliant} -- it provides atomicity, consistency, isolation, and durability
\end{itemize}
\bigskip
The EVM is the heart of Ethereum, as it executes the code of smart contracts and
...
\begin{itemize}
    \item \textbf{Imperative}, high-level language -– Code is executed step-by-step with explicit control flow (like C++).
    \item \textbf{Statically typed} -- Variable types must be declared at compile time (e.g., uint, string).
    \item \textbf{Object-oriented} -- Supports contracts (similar to classes), inheritance, interfaces, and libraries.
    \item \textbf{Smart contract focus -- Designed for writing decentralized apps (dApps)
    \item \textbf{Special features -- Built-in globals (e.g., msg.sender, block.timestamp), gas management, etc.
\end{itemize}
\bigskip
...
\end{itemize}
\url{https://https://remix.ethereum.org/}
\end{frame}
